...
|-----------|--------|------|---------|------|------|
| Accuracy | BDT | 0.840 | **0.0065** | 0.825 | 0.853 |
| Accuracy | LR | 0.852 | 0.0001 | 0.852 | 0.852 |
| AUC | BDT | 0.780 | **0.0062** | 0.764 | 0.790 |
...
安定性の比較 (標準偏差):
          ...
          BDT                    LR
  Accuracy: === 0.0065         Accuracy: | 0.0001
  AUC:      === 0.0062         AUC:      | 0.0007
...
1. **BDTは高い識別能力と十分な安定性を持つ**
   - AUCはLRを0.115ポイント上回り、ブートストラップ標準偏差は0.006（LRの0.001の8倍）
   - LRよりは変動が大きいが、絶対値としては十分小さく実用上問題ないレベル
...
- **BDT**は識別能力が高く、安定性も実用上十分なレベル（AUC σ=0.006）
