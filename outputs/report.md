...
| 指標 | 平均 | 標準偏差 | 変動係数 |
|------|------|----------|----------|
| AUC | 0.7285 | 0.0139 | 1.91% |
| Accuracy | 0.8659 | 0.0070 | 0.81% |

**示唆**: 
- AUCの標準偏差は0.014と比較的小さく、モデルの識別性能は安定している。
- Accuracyの変動は極めて小さい（CV=0.81%）が、これはクラス不均衡の影響。
- AUCのブートストラップ平均（0.729）がテスト全体での値（0.803）より低いのは、サブサンプリング（5000件）の影響。
...
2. **性能の安定性**: Accuracy CV=0.81%, AUC CV=1.91%と、データのリサンプリングに対して性能が安定。
